...
class Problema4(App):
    ...
    def generadorAleatorios(self,n0, y, mod):
        ...
        x = [n0]
        for i in range(1, ((self.preguntas + 1) * 5)):
            temp = (y*x[i-1]) % mod
            x.append(temp)
        x.pop(0)
    ...
    def preguntas_aleatorias(self):
        ...
        x0 = int(unicode_nombre(self.nombre.get()))
        y0 = int(unicode_nombre(self.materia.get()))
        z0 = int(ord(self.unidad) * self.unidad)
        ...
        for i in range(1, ((self.preguntas) * 5)):
            u = round((((x1[i-1]/30269 + y1[i-1]/30307 + z1[i-1]/30323) % 1) * 100)) + 1
            preguntas.append(u)
        preguntas_finales = list(set(preguntas))
        preguntas_finales.sort()
        
        # Mostramos resultados
        self.valores.set(preguntas_finales)
